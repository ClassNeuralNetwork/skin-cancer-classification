# %% [markdown]
# # 02 - Treinar DenseNet201 com Dataset DeepSMOTE Salvo
# 
# Este notebook não gera DeepSMOTE, ele apenas carrega e treina a DenseNet201.
...
try:
    ...
    BEST_MODEL_PATH = MODELS_DIR / "best_densenet201_deepsmote_saved_images.keras"
    FINAL_MODEL_PATH = MODELS_DIR / "densenet201_deepsmote_saved_images_final.keras"
...
try:
    def build_densenet201_classifier(input_shape=(320, 320, 3), n_classes=7):
        ...
        model = models.Model(
            inputs=inputs,
            outputs=outputs,
            name="densenet201_deepsmote_saved_images"
        )
...
try:
    ...
    plt.title("Acurácia - DenseNet201 + DeepSMOTE salvo")
    plt.xlabel("Época")
    plt.ylabel("Acurácia")
    plt.legend()
    plt.grid(True)
    plt.savefig(RESULTS_DIR / "curva_accuracy_densenet201_deepsmote_salvo.png", dpi=300, bbox_inches="tight")
    plt.show()

    plt.figure(figsize=(8, 5))
    plt.plot(history.history["loss"], label="Treino")
    plt.plot(history.history["val_loss"], label="Validação/Teste")
    plt.title("Loss - DenseNet201 + DeepSMOTE salvo")
    plt.xlabel("Época")
    plt.ylabel("Loss")
    plt.legend()
    plt.grid(True)
    plt.savefig(RESULTS_DIR / "curva_loss_densenet201_deepsmote_salvo.png", dpi=300, bbox_inches="tight")
    plt.show()
finally:
    _cell_elapsed_time = _time.time() - _cell_start_time
    _notebook_total_time = globals().get('_notebook_total_time', 0.0) + _cell_elapsed_time
...
try:
    ...
    metrics_df = pd.DataFrame([{
        "model": "DenseNet201 + DeepSMOTE saved images",
        "accuracy": acc,
    ...
    metrics_df.to_csv(RESULTS_DIR / "metricas_densenet201_deepsmote_salvo.csv", index=False)
    metrics_df
finally:
    _cell_elapsed_time = _time.time() - _cell_start_time
    _notebook_total_time = globals().get('_notebook_total_time', 0.0) + _cell_elapsed_time
...
try:
    ...
    plt.title("Matriz de Confusão (%) - DenseNet201 + DeepSMOTE salvo")
    plt.colorbar()
    ...
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / "matriz_confusao_densenet201_deepsmote_salvo.png", dpi=300, bbox_inches="tight")
    plt.show()
finally:
    _cell_elapsed_time = _time.time() - _cell_start_time
    _notebook_total_time = globals().get('_notebook_total_time', 0.0) + _cell_elapsed_time
