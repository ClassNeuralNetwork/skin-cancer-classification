# %% [markdown]
# # 02 — Treinar EfficientNetB4 com Dataset DeepSMOTE Salvo
...
try:
    ...
    BEST_MODEL_PATH = MODELS_DIR / "best_efficientnetb4_deepsmote_saved_images.keras"
    FINAL_MODEL_PATH = MODELS_DIR / "efficientnetb4_deepsmote_saved_images_final.keras"
...
try:
    def build_efficientnetb4_classifier(input_shape=(380, 380, 3), n_classes=7):
        ...
        model = models.Model(
            inputs=inputs,
            outputs=outputs,
            name="efficientnetb4_deepsmote_saved_images"
        )

        return model


    model = build_efficientnetb4_classifier(
...
try:
    ...
    plt.title("Acurácia - EfficientNetB4 + DeepSMOTE salvo")
    plt.xlabel("Época")
    plt.ylabel("Acurácia")
    plt.legend()
    plt.grid(True)
    plt.savefig(RESULTS_DIR / "curva_accuracy_efficientnetb4_deepsmote_salvo.png", dpi=300, bbox_inches="tight")
    plt.show()

    plt.figure(figsize=(8, 5))
    plt.plot(history.history["loss"], label="Treino")
    plt.plot(history.history["val_loss"], label="Validação/Teste")
    plt.title("Loss - EfficientNetB4 + DeepSMOTE salvo")
    plt.xlabel("Época")
    plt.ylabel("Loss")
    plt.legend()
    plt.grid(True)
    plt.savefig(RESULTS_DIR / "curva_loss_efficientnetb4_deepsmote_salvo.png", dpi=300, bbox_inches="tight")
    plt.show()
finally:
    _cell_elapsed_time = _time.time() - _cell_start_time
    _notebook_total_time = globals().get('_notebook_total_time', 0.0) + _cell_elapsed_time
...
try:
    ...
    metrics_df = pd.DataFrame([{
        "model": "EfficientNetB4 + DeepSMOTE saved images",
        "accuracy": acc,
    ...
    metrics_df.to_csv(RESULTS_DIR / "metricas_efficientnetb4_deepsmote_salvo.csv", index=False)
    metrics_df
finally:
    _cell_elapsed_time = _time.time() - _cell_start_time
    _notebook_total_time = globals().get('_notebook_total_time', 0.0) + _cell_elapsed_time
...
try:
    ...
    plt.title("Matriz de Confusão (%) - EfficientNetB4 + DeepSMOTE salvo")
    plt.colorbar()
    ...
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / "matriz_confusao_efficientnetb4_deepsmote_salvo.png", dpi=300, bbox_inches="tight")
    plt.show()
finally:
    _cell_elapsed_time = _time.time() - _cell_start_time
    _notebook_total_time = globals().get('_notebook_total_time', 0.0) + _cell_elapsed_time
